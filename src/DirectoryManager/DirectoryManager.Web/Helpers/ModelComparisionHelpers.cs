...
namespace DirectoryManager.Web.Helpers
{
    internal static class ModelComparisonHelpers
    {
            ...
            IReadOnlyList<string>? entryRelatedLinks = null)
        {
            ...
            void AddLinkDifference(string label, string? entryValue, string? submissionValue)
            {
                differences.Add(
                    $"<p><strong>{label}:</strong><br>" +
                    $"<em>Entry:</em><br> {FormatLink(entryValue)}<br>" +
                    $"<em>Submission:</em><br> {FormatLink(submissionValue)}</p>");
            }
            ...
            {
                AddLinkDifference("Link", entry.Link, submission.Link);
            }

            if (NotEqualTrimmed(entry.Link2, submission.Link2))
            {
                AddLinkDifference("Link2", entry.Link2, submission.Link2);
            }

            if (NotEqualTrimmed(entry.Link3, submission.Link3))
            {
                AddLinkDifference("Link3", entry.Link3, submission.Link3);
            }
